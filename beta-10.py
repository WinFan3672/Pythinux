...
os_name,app_version,="Pythinux",[0,10,0]
autologin = 0
def linux_hub():
    print("[1] APT Tools")
...
def man(manual, return_mode = 0):
    if manual == "man":
        ...
        print("MANUAL LIST")
        div()
        print("changes deprecated man / removed_man vim echo scripting todo mem updates template szips future_features stdlib alias startup consent roadmap")
        div()
        if return_mode == 0:
            main()
        else:
        ...
        div()
        print("[-] Added ihelp command")
        print("[-] Added prompt command, which changes the prompt.")
        print("[-] Disabled build-prog command [non-working output]")
        print("[-] Updated var command [now scriptable!] [spaces work now!}")
        print("[-] Added wget")
        print("[-] Added echf [echo to file]")
        print("[-] Fixed 2 major security flaws")
        print("   [-] User's SHA256 hash could be accepted as a password.")
        print("   [-] Password stored as plaintext in operating system, particularly after autologin.")
        print("      [-] As a result, getdetails no longer shows password censored.")
        print("[-] Added LinuxHub")
        br()
        if return_mode == 0:
            main()
        else:
        ...
        print("[-] Wget")
        print("Beta 11")
        print("[-] Finish User Control command")
        print("[-] Fix aliases")
        print("[-] Proper Consent Mode Integration")
        print("[-] SZIPS Compiler")
        print("[-] Files Update")
        ...
        print("[-] Remove mark command")
        print("[-] Alias support for Files")
...
def main(ch="",allow_script=0):
    ...
    ch=ch.replace("%ov_version",f"{app_version[0]}.{app_version[1]}.{app_version[2]}")
    ...
    if ch == "help":
        ...
        print("setupwizard mem ihelp prompt wget echf linuxhub")
        div()
        if return_mode == 0:
            main()
        else:
