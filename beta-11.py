...
os_name,app_version,="Pythinux",[0,11,0]
autologin = 0
def linux_hub():
    print("[1] APT Tools")
...
def man(manual, return_mode = 0):
    if manual == "man":
        ...
        div()
        print("[-] Fixed aliases properly [all commands work]")
        print("[-] Updated alias command")
        print("   [-] Added clear and remove parameters")
        print("[-] Added parameter to userlist: -c")
        print("   [-] This runs userlist_c")
        print("[-] Updated user control")
        print("[-] Added user_editor command")
        print("[-] Added ucode command")
        print("[-] One-liner support [split by ` && `]")
        print("[-] Upgraded Files to v2 [mam files2]")
        br()
        if return_mode == 0:
            main()
        else:
...
def files(startpoint,start=0,safemode=0):
    ...
    if prompt == "help":
        ...
        try:
            old=os.getcwd()
            os.chdir(prompt[3:])
            if startpoint < os.getcwd():
                pass
            else:
                print("Cannot move above startpoint")
                os.chdir(old)
        except:
            print("Bad command or file name.")
        files(startpoint,0,safemode)
...
def user_editor(buffer_data):
    ...
    if ch == "help":
        ...
        print("commit - saves changes to OS")
...
def main(ch="",allow_script=0,return_mode=-1):
    ...
    if ch == "help":
        ...
        print("setupwizard mem ihelp prompt wget echf linuxhub user_editor ucode")
        div()
        if return_mode == 0:
            main()
        else:
