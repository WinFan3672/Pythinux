...
os_name,app_version,="Pythinux",[0,12,"PR1"]
autologin = 0
def create_user(username,passw,ulvl):
    ...
    f.close()
    f=open("System Settings/userlist.pythinux","w")
    f.write(ff+f"{username}|{passw}|{ulvl}/")
    f.close()
...
def main(ch="",allow_script=0,return_mode=-1):
    ...
    if ch.startswith("build-prog") or ch.startswith("run"):
    ...
    if ch == "help":
        ...
        print("var mem ihelp prompt wget echf linuxhub user_editor ucode qaa")
        div()
        if return_mode == 0:
            main()
        else:
