...
os_name,app_version,="Pythinux",[0,12,0]
autologin = 0
import os
...
def man(manual, return_mode = 0):
    ...
    if manual == "man":
        ...
        print("   [-] Type `pkm` to get started")
        print("[-] If programs fail to run, the error is now displayed :)")
        print("[-] Added Debug Log")
        print("   [-] Type `view_log` to see a copy")
        print("   [-] Intended for debugging and troubleshooting crashes and redirects to the login screen")
        print("Added `reinstall` command")
        print("   [-] Reinstalls OS")
        print("   [-] Requires God priveleges to do")
        br()
        if return_mode == 0:
            main()
        else:
        ...
        print("[NO DOCUMENTATION INCLUDED AS OF YET]")
        br()
        if return_mode == 0:
            main()
        else:
...
def main(ch="",allow_script=0,return_mode=-1):
    ...
    if ch == "help":
        ...
        print(f"admin_panel man userlist_c vim run cat update terminal view_log")
        print(f"idle-launch jit install stdlib alias add_alias user_control pkm")
        ...
        print(f"PKM Version 1.0.0 : OS Version {app_version[0]}.{app_version[1]}.{app_version[2]}")
        if return_mode == 0:
            main()
        else:
            return True
        ...
        #print("upgrade: upgrades all upgradable packages")
        #print("add <url>: adds a database from <url>")
        #print("db: manipulates installed DB's.")
        print("version: displays version information about pkm")
        print("verison -c: shows less detailed information about pkm")
        div()
        if return_mode == 0:
            main()
        else:
        ...
        for item in packages:
            if item[0] == term:
                if item[2] == "about:blank":
                    ...
                    print("To fix this, contact the program developer.")
                    if return_mode == 0:
                        main()
                    else:
                        return True
        ...
        if crashlog == "Pythinux Errors:\n":
            print("Pythinux Errors:\nN/A")
        else:
            print(crashlog)
        if return_mode == 0:
            main()
