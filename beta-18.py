...
PREFERENCES = {
    "editor_comment":f"When editing this file, exercise caution. To reset your progress, you can delete the file. It will be rebuilt.",
    "key":{
            "pref_format":"ersion of preferences file. DO NOT EDIT!",
            "time_format":"Time format used by commands.",
            "div":"div element used",
            "alias_priority":"If True, aliases are executed before commands are",
            "default_prompt":"Currently broken.",
            "banlist":"List of commands inaccessible to guest users.",
        },
    "pref_format": [1, 5, 0],
...
def man(manual, return_mode=0):
    ...
    if manual == "man":
        ...
        div()
        print("[-] Added CALC command")
        print("[-] Added FORMAT command")
        print("[-] Using [{}] brackets now works with 2+ brackets")
        print("[-] Added SORTED command")
        print("[-] PKM Update Errors added to crash log")
        print("[-] Guest users cannot use certain commands (see \"banlist\" command for list)")
        print("[-] Added -fc switch for \"about\" command")
        print("[-] Added EHELP command")
        print("[-] variable brackets or \"{}\" are now processed first in main()")
        print("[-] Added Tron text editor")
        print("[-] SCRIPT command updated")
        print("   [-] SCRIPT LIST updated to only show scripts and hide extension")
        print("   [-] Added SCRIPT TRON")
        print("[-] Files 2.3")
        print("    [-] Added TRON command")
        print("[-] Added DaVinci bitmap editor")
        print("[-] Added functions")
        print("   [-] funct command defines commands")
        print("   [-] call command calls functions")
        print("[-] Added custom manual support")
        print("   [-] \"man cman\"")
        print("[-] Added BREAK command")
        br()
    elif manual == "pkm":
        print(
...
def refresh_pref():
    ...
    if os.path.isfile("System Settings/system.preferences") == False:
        save_pref(PREFERENCES)
    with open("System Settings/system.preferences","r") as f:
        d=f.read().splitlines()
...
def save_pref(preferences):
    ...
    with open("System Settings/system.preferences", "w") as f:
        f.write(pprint_dict(obj_to_dict(preferences)))
