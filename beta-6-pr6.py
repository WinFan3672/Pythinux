...
os,app_version,="Pythinux",[0,6,"PR6"]
autologin = 0
def man(manual):
    print("[NO MANUALS INSTALLED]")
    main()
def files():
        import os
        safemode=0
...
def main():
    ...
    if ch == "help":
        ...
        div()
        print(f"about help logoff author rewrite mul rand rng time cls")
        ...
        print(f"scriptux admin_panel files man userlist_c")
        div()
        main()
    elif ch == "about":
        div()
        ...
        if is_root() == True:
            ...
            print(f"This build is not 100% tested and was ported to the\nnew {os} from the old one.")
            div()
            files()
        else:
        ...
        not_update_text = f"An update for {os} [{data[0]}.{data[1]}.{data[2]}] is available.\nNo download available"
        if data[0] > app_version[0]:
            ...
            if data[1] > app_version[1]:
                ...
                if data[2] > app_version[2]:
                    ...
                    print(f"You are on the latest version of {os}.")
        main()
    else:
        print(f"{upper(ch)} IS NOT A VALID COMMAND, PROGRAM OR MANUAL.")
        main()
