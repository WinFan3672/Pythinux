...
os_name,app_version,="Pythinux",[0,6,"PR7"]
autologin = 1
def vim_editor(fn=""):
    if fn == "":
...
def man(manual):
    if manual == "man":
        ...
        print("[-] To see {os_name}'s changelog, type MAN CHANGES.")
        br()
        main()
    elif manual == "rewrite":
        div()
...
def scriptux():
    ...
    div()
    print(f"This version of Scriptux may have compatibility issues with the new {os_name}.")
    div()
...
def main():
    ...
    if ch == "help":
        ...
        print(f"scriptux admin_panel files man userlist_c vim")
        div()
        main()
    elif ch == "about":
        div()
